...
**Repo :** `app-allo-debouchage` — outil interne Next.js 14 App Router qui transforme la dictée vocale d'un technicien plombier (Allo Débouchage, Var) en :
1. Un rapport d'intervention (PDF + page de réalisation SEO publiée sur allodebouchage.com).
...
## Règles non-négociables (Allo Débouchage)
...
2. **Téléphone :** s'il apparaît dans du code, doit être `+33 7 83 63 68 35` (variable d'env `ALLO_PHONE` si possible). Ne pas inventer.
3. **Prix :** ne jamais hardcoder. Les placeholders `{PRIX_MIN}` / `{PRIX_MAX}` dans les prompts existants restent en place.
4. **Site web :** `https://allodebouchage.com` (constante `SITE` dans `generate/route.ts`).
...
- Quotas Upstash free tier : 10K commandes / jour, 256 MB. Pour Allo Débouchage (~50-200 calls IA / jour estimés), c'est largement suffisant.
