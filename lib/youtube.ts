...
export async function buildVideoMetadata(opts: {
  ...
  const title = `${type} à ${ville} — Allo Débouchage Plombier 24h/24${ref}`

  const summary =
    opts.rapport?.resume_court ||
    opts.rapport?.resume ||
    `Intervention de ${type.toLowerCase()} réalisée à ${ville} par Allo Débouchage.`
  ...
  const description = [
    summary,
    "",
    "🔧 Allo Débouchage — 24h/24, 7j/7",
    `📞 ${tel}`,
    "🌐 https://allodebouchage.com",
    "📍 Toulon, Hyères, Fréjus, Draguignan, Brignoles et tout le Var",
    "",
    "Tarif débouchage à partir de 99€ TTC.",
    "",
    `#debouchage #plomberie #${ville.toLowerCase().replace(/\s+/g, "")} #var #urgence #allodebouchage`,
  ].join("\n")
