...
import { ALLO_EMETTEUR, alloFactureEmetteur } from "@/lib/emetteur"
...
export default function FacturePage() {
  ...
  useEffect(() => {
    ...
    const raw = sessionStorage.getItem('allo_devis_to_facture')
    if (!raw) return
    try {
      const payload = JSON.parse(raw)
      sessionStorage.removeItem('allo_devis_to_facture')
  ...
  async function handleSendToClient() {
    ...
    try {
      ...
      const technicienNom = typeof window !== 'undefined' ? (localStorage.getItem('allo_technicien') || '') : ''
      ...
      const emetteur = alloFactureEmetteur(agence)
