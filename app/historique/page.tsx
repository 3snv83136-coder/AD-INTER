                ...
                <tbody>
                  {interventions.map(i => (
                    <tr key={i.id} className={`border-t border-slate-100 hover:bg-slate-50 ${deletingId === i.id ? 'opacity-50' : ''}`}>
                      ...
                      <td className="px-4 py-3">
                        {i.publie_slug ? (
                          <a href={`https://allodebouchage.com/nos-realisations/${i.publie_slug}`} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline font-semibold text-xs">
                            voir →
                          </a>
                        ) : (
