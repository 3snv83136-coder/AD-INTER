...
function notifyDone(title: string, body: string) {
  ...
  try { new Notification(title, { body, icon: '/icon-192.png', tag: 'allo-rapport' }) } catch {}
}
...
export default function NouveauPage() {
  ...
  // Pré-remplissage depuis Planning (sessionStorage 'allo_intervention_prefill')
  // ou chargement direct d'un rapport existant pour édition (sessionStorage 'allo_load_rapport_id').
  useEffect(() => {
    if (typeof window === 'undefined') return
    const loadId = sessionStorage.getItem('allo_load_rapport_id')
    if (loadId) {
      sessionStorage.removeItem('allo_load_rapport_id')
      sessionStorage.removeItem('allo_intervention_prefill')
    ...
    const raw = sessionStorage.getItem('allo_intervention_prefill')
    if (!raw) return
    try {
  ...
  useEffect(() => {
    const saved = typeof window !== 'undefined' ? localStorage.getItem('allo_technicien') : null
    if (saved) setTechnicienNom(saved)
    else setEditTech(true)
  }, [])
  useEffect(() => {
    if (technicienNom && typeof window !== 'undefined') localStorage.setItem('allo_technicien', technicienNom)
  }, [technicienNom])

  // Restauration du brouillon au mount (sauf si un prefill /planning est présent)
  useEffect(() => {
    if (typeof window === 'undefined') return
    if (sessionStorage.getItem('allo_intervention_prefill')) return
  ...
  function handleCreateFacture() {
    ...
    sessionStorage.setItem('allo_devis_to_facture', JSON.stringify(payload))
  ...
  async function handlePublish() {
    ...
    // Django Allo Débouchage exige technicien_name NOT NULL (sinon IntegrityError 500).
      ...
      <main className="max-w-3xl mx-auto px-4 py-5 space-y-4">
        ...
        {step === 'done' && (
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 text-center space-y-5">
            ...
            <a
              href={`https://allodebouchage.com/nos-realisations/${publishedSlug}`}
              target="_blank"
