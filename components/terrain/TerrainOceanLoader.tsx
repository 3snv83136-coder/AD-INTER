...
export default function TerrainOceanLoader({ done = false, expectedMs = DEFAULT_EXPECTED_MS }: Props) {
  ...
  return (
    <div className="space-y-3">
      <style>{`
        @keyframes allo-wave1 { 0% { transform: translateX(0); } 100% { transform: translateX(-50%); } }
        @keyframes allo-wave2 { 0% { transform: translateX(0); } 100% { transform: translateX(-50%); } }
        @keyframes allo-sun-pulse {
          0%, 100% { transform: scale(1); filter: brightness(1); }
          50% { transform: scale(1.08); filter: brightness(1.15); }
        }
        @keyframes allo-boat-bob {
          0%, 100% { transform: translateY(0) rotate(-2deg); }
          50% { transform: translateY(-4px) rotate(2deg); }
        }
        .allo-wave-1 { animation: allo-wave1 4s linear infinite; }
        .allo-wave-2 { animation: allo-wave2 6s linear infinite; }
        .allo-sun { animation: allo-sun-pulse 3s ease-in-out infinite; }
        .allo-boat { animation: allo-boat-bob 2s ease-in-out infinite; }
      `}</style>
      ...
      >
        <div
          className="allo-sun absolute w-16 h-16 sm:w-20 sm:h-20 rounded-full"
          style={{
            top: '8px',
            right: '12px',
        ...
        >
          <svg
            className="allo-wave-1 absolute top-0 left-0 h-6"
            style={{ width: '200%' }}
            viewBox="0 0 1200 40"
          ...
          <svg
            className="allo-wave-2 absolute top-2 left-0 h-5"
            style={{ width: '200%' }}
            viewBox="0 0 1200 40"
            preserveAspectRatio="none"
          >
        ...
        >
          <span className="allo-boat inline-block">⛵</span>
        </div>
