...
export default function CatLoader() {
  return (
    <div className="relative h-24 w-full overflow-hidden rounded-xl bg-gradient-to-b from-sky-200 via-sky-100 to-indigo-50 border border-sky-200">
      <div className="allo-cloud absolute" style={{ top: 8, width: 28, height: 8 }}>
        <div className="w-full h-full rounded-full bg-white/80" />
      </div>
      <div className="allo-cloud-2 absolute" style={{ top: 22, width: 36, height: 9 }}>
        <div className="w-full h-full rounded-full bg-white/70" />
      </div>
      <div className="allo-cloud-3 absolute" style={{ top: 50, width: 22, height: 6 }}>
        <div className="w-full h-full rounded-full bg-white/60" />
      </div>

      <div className="allo-cat absolute" style={{ width: 78, top: '30%' }}>
        <svg viewBox="0 0 78 40" className="w-[78px] h-10 overflow-visible">
          <g className="allo-cape">
            <path
      ...
      <style>{`
        ...
        .allo-cat { animation: alloCatFly 6s ease-in-out infinite; transform-origin: center; }

        @keyframes alloBob {
          0%, 100% { transform: translateY(0); }
          50%      { transform: translateY(-2px); }
        }
        .allo-cat svg { animation: alloBob 0.7s ease-in-out infinite; }

        @keyframes alloCapeWave {
          0%, 100% { transform: rotate(0deg) translateX(0); }
          50%      { transform: rotate(-6deg) translateX(-1.5px); }
        }
        .allo-cape { animation: alloCapeWave 0.4s ease-in-out infinite; transform-origin: 12px 22px; transform-box: fill-box; }

        @keyframes alloTailWiggle {
          0%, 100% { transform: rotate(0deg); }
          50%      { transform: rotate(8deg); }
        }
        .allo-tail { animation: alloTailWiggle 0.45s ease-in-out infinite; transform-origin: 56px 20px; transform-box: fill-box; }

        @keyframes alloSparkle {
          0%, 100% { opacity: 0; transform: scale(0.4); }
          50%      { opacity: 1; transform: scale(1); }
        }
        .allo-sparkle { animation: alloSparkle 1s ease-in-out infinite; transform-box: fill-box; transform-origin: center; }
        .allo-sparkle-2 { animation-delay: 0.3s; }
        .allo-sparkle-3 { animation-delay: 0.6s; }

        @keyframes alloCloudDrift {
          from { left: 110%; }
          to   { left: -20%; }
        }
        .allo-cloud   { animation: alloCloudDrift 9s  linear infinite; }
        .allo-cloud-2 { animation: alloCloudDrift 13s linear infinite; animation-delay: -3s; }
        .allo-cloud-3 { animation: alloCloudDrift 7s  linear infinite; animation-delay: -1.5s; }
      `}</style>
    </div>
  )
}
