...
function getTechnicien(): string {
  ...
  return localStorage.getItem('allo_technicien') || ''
}
...
async function buildSendBody(
  ...
  if (doc.type === 'facture') {
    ...
    const emetteur = alloFactureEmetteur(doc.agence || undefined)
  ...
  if (doc.type === 'devis') {
    ...
    const element = React.createElement(DevisDocument, {
      emetteur: ALLO_EMETTEUR, client: buildClientData(doc), devis, phone: ALLO_EMETTEUR.telephone,
    })
    const pdfBase64 = await pdfDocumentToBase64(element)
