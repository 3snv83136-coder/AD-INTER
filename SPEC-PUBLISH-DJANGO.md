...
> Document de contrat entre le **back-office Next.js** (`app-allo-debouchage`)
> et le **site Django** (`allodebouchage.com`).
...
```
POST {ALLO_API_URL}/api/gallery/publish/
Authorization: Bearer {ALLO_PUBLISH_TOKEN}
```
...
- **Le service rendu** : `Service` — `name` = `service_type`,
  ...
  `provider` = le `LocalBusiness` Allo Débouchage.
...
- Présentation Allo Débouchage (activité, zone : Var, téléphone).
